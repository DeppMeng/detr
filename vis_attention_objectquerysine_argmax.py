...
for idxx, img_id in enumerate(id_list):
    ...
    for count in range(1):
        ...
        att_weights = torch.mul(obj_embed.T, enc_pos)
        ...
        if count == 0:
            ...
            for idx, ax_i, (xmin, ymin, xmax, ymax) in zip(keep.nonzero(), axs.T, bboxes_scaled):
                ax = ax_i[0]
                ax.imshow(sum_argmax_attn.view(h, w))
                ax.axis('off')
            ...
            plt.savefig('vis_attn_v4_sineobjquery/idx{}_layer{}_objectquerysine_only_pos_avg.png'.format(img_id, 0), format='png')